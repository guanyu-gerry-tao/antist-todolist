...
function Task({ task, tasks, }: { task: [TaskId, TaskType], tasks: [TaskId, TaskType][], }) {
  ...
  const isPending = task[1].isPending || false;
  ...
  // Handle confirming a pending task
  const handleConfirmPending = async (e: React.MouseEvent<HTMLButtonElement>) => {
    e.stopPropagation();
    ...
    try {
      actions.updateTask({ id: task[0], updatedFields: { isPending: false } }, backup);
      optimisticUIUpdate(setStates, backup);
      postPayloadToServer('/api/bulk', navigate, backup);
      console.log(`Confirmed pending task: ${task[1].title}`);
    } catch (error) {
  ...
  // Handle dismissing a pending task
  const handleDismissPending = async (e: React.MouseEvent<HTMLButtonElement>) => {
    e.stopPropagation();
    ...
    try {
      actions.deleteTask(task[0], backup);
      optimisticUIUpdate(setStates, backup);
      postPayloadToServer('/api/bulk', navigate, backup);
      console.log(`Dismissed pending task: ${task[1].title}`);
    } catch (error) {
      console.error('Error dismissing pending task:', error);
      restoreBackup(setStates, backup);
    }
  };
            ...
            >
              <div
                ...
                style={{
                  ...
                  opacity: isPending ? 0.6 : 1, // Semi-transparent for pending tasks
                  filter: isPending ? 'brightness(0.9)' : 'none' // Slightly dimmed for pending tasks
                }}
              >
